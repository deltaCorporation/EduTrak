...
if($user->isLoggedIn()){
    if($user->isSetup()) {
        ...
        <html lang="en">
        <head>
            <?php include __DIR__ . '/include/meta.php'; ?>
        </head>
        <body>
        ...
        <div id="board-wrapper">
            ...
            <div id="index-board-header" class="index-board-block block-1">
                ...
                <ul>
                    <li>
                        <button data-link="main" type="button" onclick="changeDashboard('main', this)"><i
                                    class="fas fa-th-list"></i></button>
                    </li>
                    <?php if ($user->hasPermission('sales')): ?>
                        <li>
                            <button data-link="kanban" type="button" onclick="changeDashboard('kanban', this)"><i
                                        class="fab fa-trello"></i></button>
                        </li>
                    <?php endif; ?>
                </ul>
            </div>
        ...
        <!-- DASHBOARD HEADER END -->
            ...
            <section data-type="kanban" id="kanban-board" class="board">
                ...
                <?php foreach ($user->getKanbanColumns($user->data()->section) as $column): ?>
                    ...
                    <section data-id="<?php echo $column->ID ?>" data-kanban="column" ondrop="drop(event)"
                             ondragover="allowDrop(event)">
                        <h4 data-kanban="header"><?php echo $column->name ?></h4>
                        <button class="kanban-new-request" data-kanban="new-request"
                                data-request-status="<?php echo $column->ID ?>">
                            <i class="fas fa-plus"></i>
                        </button>
                        <?php foreach ($user->getKanbanRequests($column->ID, $user->data()->id) as $item): ?>
                            <div id="<?php echo $item->ID ?>" data-kanban="item" draggable="true"
                                ...
                                <div class="activity-log-section-board">
                                    ...
                                    <div class="activity-log-content-board">
                                        <?php foreach ($logs as $log): ?>
                                            ...
                                            <a href="info.php?case=<?php echo $log['case'] ?>&id=<?php echo $log['caseID'] ?>"
                                               class="activity-log-board">
                        ...
                        <?php if ($companies = $leads->getFollowUpLeads()): ?>
                            ...
                            <?php foreach ($companies as $company): ?>
                                <?php if ($company->followUpDate > date('Y-m-d')): ?>
                                    <a href='info.php?case=<?php echo $company->caseName ?>&id=<?php echo $company->ID ?>'
                                       class='index-board-followUp-item'>
                            ...
                            <?php foreach ($events->getUpcomingEvents() as $event): ?>
                                ...
                                <?php if ($newDate >= date('Ymd')): ?>
                                    <?php if ($i < 5): ?>
                                        ...
                                        <a href='info.php?case=customer&id=<?php echo $event->customerID ?>&tab=event'
                                           class='index-board-last-event-wrapper'>
                                            <div>
        ...
        <div class="flash-msg <?php if (Session::exists('home')) {
            echo 'show-msg';
        } ?>">
            <?php
...
<!--                    </label>-->
                    <a class="forgot-pass-link" href="forgotPassword.php">Forgot password?</a>
